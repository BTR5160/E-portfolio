    ...
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="relative">
          {/* Timeline Line */}
          <div className={`absolute left-8 md:left-1/2 transform md:-translate-x-1/2 w-1 h-full ${darkMode ? 'bg-gray-600' : 'bg-gray-300'}`}></div>
          ...
          <div className="space-y-12">
            {experiences.map((exp, index) => (
              ...
              >
                {/* Timeline Node */}
                <div className={`absolute left-8 md:left-1/2 transform -translate-x-1/2 w-6 h-6 rounded-full border-4 ${darkMode ? 'bg-gray-800 border-gray-600' : 'bg-white border-gray-300'} z-10`}></div>
                <div className={`absolute left-8 md:left-1/2 transform -translate-x-1/2 w-6 h-6 rounded-full bg-gradient-to-r ${getTypeColor(exp.type)}`}></div>

                {/* Content Card */}
                <div className={`ml-20 md:ml-0 md:w-5/12 ${index % 2 === 0 ? 'md:mr-8' : 'md:ml-8'}`}
                >
                  <div className={`p-6 rounded-xl shadow-lg transition-all duration-300 hover:shadow-xl ${darkMode ? 'bg-gray-700' : 'bg-white'} border ${darkMode ? 'border-gray-600' : 'border-gray-200'}`}
                  >
                    ...
                    <h4 className={`text-lg font-semibold mb-3 ${darkMode ? 'text-[#577B87]' : 'text-[#294149]'}`}>{exp.company}</h4>
                    ...
                    <p className={`text-base leading-relaxed ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>{exp.description}</p>
                  </div>
                </div>
              </div>
            ))}
